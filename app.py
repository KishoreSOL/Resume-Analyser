...
def screening(name):
    if request.method == 'POST':
        # Process file upload in POST request
        if 'file' not in request.files:
            flash('No file part')
            return redirect(request.url)
        file = request.files['file']
    ...
    # Reading and processing the PDF file
    ...
    with open(file_path, 'rb') as pdfFileObj:
        ...
        for count in range(num_pages):
            ...
            text += pageObj.extract_text() if pageObj.extract_text() else ""
    ...
    ax.set_aspect('equal')
    ax.set_title("Skill scores in each field.")
    buf = BytesIO()
